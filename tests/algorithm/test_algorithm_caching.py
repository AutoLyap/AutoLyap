...
def test_cached_matrices_are_readonly(constant_algorithm):
    sys_mats = constant_algorithm.get_AsBsCsDs(0, 0)
    A = sys_mats[0][0]
    with pytest.raises(ValueError):
        A[0, 0] = 2.0

    Ps = constant_algorithm.get_Ps()
    with pytest.raises(ValueError):
        Ps[(1, 1)][0, 0] = 1.0


def test_cache_invalidation_non_structural_keeps_static(constant_algorithm):
    constant_algorithm.get_Us(0, 0)
    constant_algorithm.get_AsBsCsDs(0, 0)
