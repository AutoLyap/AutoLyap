#!/usr/bin/env python3
"""Generate heavy-ball (gradient-dominated + smooth) sweep data and an SVG plot.
...
DATA_REL = (
    Path("data")
    / "heavy_ball_gradient_dominated_smooth"
    / "gammas_deltas_rho.csv"
)
PLOT_IMAGE_REL = Path("_static") / "heavy_ball_gradient_dominated_smooth.svg"
...
GAMMA_SWEEP = (0.01, 2.30, 100)
DELTA_SWEEP = (-1.00, 1.00, 100)
...
PLOT_X_RANGE = (0.0, 2.30)
PLOT_Y_RANGE = (-1.0, 1.0)
PLOT_X_TICKS = (0.0, 0.5, 1.0, 1.5, 2.0, 2.3)
...
PLOT_SHOW_GRID = True
PLOT_TITLE = "Certified heavy-ball linear rates (gradient-dominated + smooth)"
PLOT_DESCRIPTION = (
    "Rho over feasible heavy-ball (gamma, delta) pairs for gradient-dominated, smooth objectives."
)
PLOT_ARIA_LABEL = "Heavy-ball rho over gamma and delta"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Generate heavy-ball (gradient-dominated + smooth) sweep data and SVG plot assets."
        )
    )
    parser.add_argument(
        "--output-dir",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.reuse_data:
        ...
        print("Running heavy-ball gradient-dominated sweep...")
