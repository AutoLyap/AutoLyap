#!/usr/bin/env python3
"""Generate ITEM sweep data and a publication-style SVG plot asset.
...
DATA_REL = Path("data") / "information_theoretic_exact_method" / "k_c.csv"
PLOT_IMAGE_REL = (
    Path("_static") / "information_theoretic_exact_method_c_vs_K_loglog.svg"
)
...
PLOT_TITLE = "Information-theoretic exact method bound vs iteration budget (log-log)"
PLOT_DESCRIPTION = (
    "Log-log c_K vs K for ITEM: theoretical curve and AutoLyap points."
)
PLOT_ARIA_LABEL = "ITEM c_K versus K in log-log scale"
PLOT_SHOW_GRID = True
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Generate ITEM c_K-vs-K data and SVG plot assets."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def _validate_parameters(mu: float, L: float, k_min: int, k_max: int) -> None:
    if not (mu > 0.0):
    ...
    if k_max < k_min:
        raise ValueError(f"Require k_max >= k_min. Got k_min={k_min}, k_max={k_max}.")
...
def _split_Q_q(
    parameters: np.ndarray | Tuple[np.ndarray, np.ndarray],
...
def _c_K_theory(q: float, A_tilde_k: float) -> float:
    return 1.0 / (1.0 + q * A_tilde_k)
...
def _run_scan(
    ...
    for row_id, K in enumerate(ks, start=1):
        k_int = int(K)
        A_tilde_k = algorithm.get_A(k_int)
        c_K_theory = _c_K_theory(q=q, A_tilde_k=A_tilde_k)
        Q_K, q_K = _split_Q_q(
            IterationDependent.get_parameters_state_component_distance_to_solution(
                algorithm,
                k=k_int,
...
def _render_plot(
    ...
    theory_points_raw = [
        (float(K), float(c_K_theory))
        for K, _, c_K_theory in rows
        if c_K_theory > 0.0
    ]
    autolyap_points_raw = [
    ...
    all_c_K_values = [c_K for _, c_K in theory_points_raw] + [
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    _validate_parameters(mu=args.mu, L=args.L, k_min=args.k_min, k_max=args.k_max)
    ...
    print(f"mu={args.mu}")
    print(f"L={args.L}")
    print(f"K grid: {len(ks)} points on [{int(ks[0])}, {int(ks[-1])}]")
    print()
    ...
    if args.reuse_data:
        ...
        print("Running ITEM c_K sweep...")
        rows, errors = _run_scan(ks, args.mu, args.L, solver_options)
        data_path = args.output_dir / DATA_REL
