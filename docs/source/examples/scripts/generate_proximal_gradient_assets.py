#!/usr/bin/env python3
"""Generate proximal-gradient sweep data and a publication-style SVG plot asset.
...
DATA_REL = Path("data") / "proximal_gradient" / "gamma_rho.csv"
PLOT_IMAGE_REL = Path("_static") / "proximal_gradient_rho_vs_gamma.svg"
...
PLOT_TITLE = "Proximal-gradient contraction factor vs step size"
PLOT_DESCRIPTION = "Rho vs gamma for proximal gradient: theoretical curve and AutoLyap points."
PLOT_ARIA_LABEL = "Proximal-gradient rho versus gamma"
PLOT_SHOW_GRID = True
...
class ProximalGradientMethod(Algorithm):
    """Concrete proximal-gradient method in the base ``Algorithm`` state-space form."""
    ...
    def get_ABCD(
        self,
        k: int,  # noqa: ARG002 - time-invariant dynamics.
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        A = np.array([[1.0]])
        B = np.array([[-self.gamma, -self.gamma]])
        C = np.array([[1.0], [1.0]])
        D = np.array([[0.0, 0.0], [-self.gamma, -self.gamma]])
        return A, B, C, D
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Generate proximal-gradient rho-vs-gamma data and SVG plot assets."
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def _run_scan(
    ...
    problem = InclusionProblem([SmoothStronglyConvex(mu, L), Convex()])
    algorithm = ProximalGradientMethod(gamma=float(gammas[0]))
    P, p, T, t = (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.reuse_data:
        ...
        print("Running proximal-gradient rho sweep...")
