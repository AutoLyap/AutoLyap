...
DATA_REL = (
    Path("data")
    / "douglas_rachford_maximally_monotone_lipschitz_plus_strongly_monotone"
    / "lambda_rho.csv"
)
PLOT_IMAGE_REL = (
    Path("_static")
    / "douglas_rachford_maximally_monotone_lipschitz_plus_strongly_monotone_rho_vs_lambda.svg"
)
...
PLOT_Y_RANGE = (0.6, 1.0)
PLOT_Y_TICKS = (0.6, 0.7, 0.8, 0.9, 1.0)
PLOT_WIDTH_PX = 960
...
def _rho_theoretical(mu: float, L: float, lambda_value: float, gamma: float) -> float:
    """Piecewise theoretical rho from Ryu et al., Theorem 4.3."""
    if gamma <= 0.0:
    ...
    if not (scaled_mu > 0.0 and scaled_L > 0.0 and 0.0 < lambda_value < 2.0):
    ...
    condition_b_threshold = (
        ...
        * (scaled_mu + scaled_mu * scaled_L**2 - scaled_L**2 - 2.0 * scaled_mu * scaled_L - 1.0)
        ...
        - scaled_L**3
        - 3.0 * scaled_mu * scaled_L**2
        - scaled_L**2
        - 2.0 * scaled_mu**2 * scaled_L
        - scaled_mu * scaled_L
        - scaled_L
        - 1.0
    )
    if (
        ...
        return abs(
            1.0
            - lambda_value * (scaled_L + scaled_mu) / ((scaled_mu + 1.0) * (scaled_L + 1.0))
        ) ** 2

    term3 = lambda_value * (scaled_L**2 + 1.0) - 2.0 * scaled_mu * (
        lambda_value + scaled_L**2 - 1.0
    )
    term4 = lambda_value * (1.0 + 2.0 * scaled_mu + scaled_L**2) - 2.0 * (
        scaled_mu + 1.0
    ) * (scaled_L**2 + 1.0)
    numerator = (2.0 - lambda_value) * term3 * term4
    denominator = 4.0 * scaled_mu * (scaled_L**2 + 1.0) * (
...
def _run_scan(
    ...
    for row_id, lambda_value in enumerate(lambda_values, start=1):
        ...
        try:
            rho_theory = _rho_theoretical(mu, L, lambda_float, gamma)
...
def _render_plot(
    ...
    theory_points.extend(
        ...
        (
            float(lambda_value),
            _rho_theoretical(mu, L, float(lambda_value), gamma),
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    _validate_parameters(mu=args.mu, L=args.L, gamma=args.gamma)
