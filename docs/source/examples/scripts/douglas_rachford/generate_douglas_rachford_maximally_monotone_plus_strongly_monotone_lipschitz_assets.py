...
DATA_REL = (
    Path("data")
    / "douglas_rachford_maximally_monotone_plus_strongly_monotone_lipschitz"
    / "gamma_rho.csv"
)
PLOT_IMAGE_REL = (
    Path("_static")
    / "douglas_rachford_maximally_monotone_plus_strongly_monotone_lipschitz_rho_vs_gamma.svg"
)
...
def _dr_operator_delta(mu: float, L: float, gamma: float) -> float:
    radicand = 1.0 - (4.0 * gamma * mu) / (1.0 + 2.0 * gamma * mu + (gamma * L) ** 2)
    return float(np.sqrt(max(radicand, 0.0)))


def _dr_operator_rate_sq(lambda_value: float, mu: float, L: float, gamma: float) -> float:
    alpha = lambda_value / 2.0
    delta = _dr_operator_delta(mu, L, gamma)
...
def _run_scan(
    ...
    for row_id, gamma in enumerate(gammas, start=1):
        ...
        rho_theory = _dr_operator_rate_sq(lambda_value, mu, L, gamma_float)
...
def _render_plot(
    ...
    theory_points = [
        (float(gamma), _dr_operator_rate_sq(lambda_value, mu, L, float(gamma)))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    _validate_parameters(mu=args.mu, L=args.L, lambda_value=args.lambda_value)
