...
DATA_REL = Path("data") / "douglas_rachford_operator_giselsson_thm65" / "gamma_rho.csv"
PLOT_IMAGE_REL = (
    Path("_static") / "douglas_rachford_operator_giselsson_thm65_rho_vs_gamma.svg"
)
...
def _run_scan(
    ...
    for row_id, gamma in enumerate(gammas, start=1):
        ...
        rho_theory = _giselsson_thm65_rate_sq(lambda_value, mu, L, gamma_float)
...
def _render_plot(
    ...
    theory_points = [
        (float(gamma), _giselsson_thm65_rate_sq(lambda_value, mu, L, float(gamma)))
