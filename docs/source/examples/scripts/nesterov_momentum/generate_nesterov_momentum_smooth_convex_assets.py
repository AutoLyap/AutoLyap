#!/usr/bin/env python3
"""Generate constant-Nesterov-momentum smooth-convex sweep data and an SVG.
...
SMOOTH_DATA_REL = (
    Path("data") / "nesterov_momentum_smooth_convex" / "gammas_deltas.csv"
)
PLOT_IMAGE_REL = Path("_static") / "nesterov_momentum_smooth_convex.svg"

# Single sweep grid: (min, max, count).
GAMMA_SWEEP = (0.01, 3.50, 100)
DELTA_SWEEP = (-1, 1, 100)
...
PLOT_X_RANGE = (0.0, 3.5)
PLOT_Y_RANGE = (-1.0, 1.0)
PLOT_X_TICKS = (0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5)
PLOT_Y_TICKS = (-1.0, -0.5, 0.0, 0.5, 1.0)
PLOT_X_LABEL = r"$\gamma$"
PLOT_Y_LABEL = r"$\delta$"
PLOT_Y_LABEL_ROTATION_DEG = 0
PLOT_SHOW_GRID = True
PLOT_TITLE = "Certified constant-Nesterov-momentum smooth-convex region"
PLOT_DESCRIPTION = "Scatter plot of feasible parameter pairs in the (gamma, delta) plane."
PLOT_ARIA_LABEL = "Constant-Nesterov-momentum certified parameter region"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Generate constant-Nesterov-momentum smooth-convex sweep data and "
            "SVG plot assets."
        )
    )
    parser.add_argument(
